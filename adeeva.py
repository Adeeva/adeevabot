...
#line = LINE()
line = LINE("Eq28ocdVRoRiymR0nQp7.V1wijKfG42+xcW7/M8LfDW.pv6B70xUf/xP82sIuLpPSTzd16xxpw/dM4noOnx1kSA=")
...
def helpmessage():
    helpMessage = "╔══[─┅═✥[Adeeva]✥═┅─]" + "\n" + \
                  ...
                  "╚══[ᴡʏᴠᴇʀɴ-ɪᴅ & ᴜɴɪᴛʏ ᴛᴇᴀᴍ]"
    return helpMessage
    
def helptexttospeech():
    helpTextToSpeech =   "╔══[🐝Myeisha🐝]" + "\n" + \
                         "╠ af : Afrikaans" + "\n" + \
                         "╠ sq : Albanian" + "\n" + \
                         "╠ ar : Arabic" + "\n" + \
                         "╠ hy : Armenian" + "\n" + \
    ...
    try:
        ...
        if op.type == 5:
            ...
            if settings["autoAdd"] == True:
                line.sendMessage(op.param1, "Halo {} Thanks For Add.".format(str(line.getContact(op.param1).displayName)))
        if op.type == 13:
        ...
        if op.type == 25:
            ...
            if msg.contentType == 0:
                if text is None:
                    return
#==============================================================================#
                if text.lower() == 'adeeva':
                    helpMessage = helpmessage()
                    line.sendMessage(to, str(helpMessage))
                    line.sendContact(to, "ubf5588264160e36fd70b0efe9bc94577")
                elif text.lower() == 'myeisha':
                    helpTextToSpeech = helptexttospeech()
                    line.sendMessage(to, str(helpTextToSpeech))
                elif text.lower() == 'radella':
                    helpTranslate = helptranslate()
                    line.sendMessage(to, str(helpTranslate))
#==============================================================================#
                elif text.lower() == 'lari':
                    start = time.time()
                    line.sendMessage(to, "tunggu ya kak…")
                    elapsed_time = time.time() - start
                    line.sendMessage(to,format(str(elapsed_time)))
                elif text.lower() == 'restart':
                    line.sendMessage(to, "In Progress")
                    line.sendMessage(to, "Done")
                    restartBot()
                elif text.lower() == 'waktuku':
                    timeNow = time.time()
                    runtime = timeNow - botStart
                    runtime = format_timespan(runtime)
                    line.sendMessage(to, "Kita sudah menikah selama {}".format(str(runtime)))
                elif text.lower() == 'suamiku':
                    try:
                        arr = []
                        owner = "ubf5588264160e36fd70b0efe9bc94577"
...
#==============================================================================#
                    ...
                    if 'MENTION' in msg.contentMetadata.keys()!= None:
                        ...
                        try:
                            ...
                            line.sendMessage(msg.to, "Done To Clone. Becarefull, Dont Use it for Criminal")
                        except:
                            line.sendMessage(msg.to, "Failed To Clone")
                    ...
                    try:
                        ...
                        line.sendMessage(msg.to, "Restore Done")
                    except:
                        line.sendMessage(msg.to, "Failed To Restore. You Trapped")
#==============================================================================#
                    ...
                    for target in targets:
                        try:
                            ...
                            line.sendMessage(msg.to,"Added Target!")
                            break
                        except:
                    ...
                    for target in targets:
                        try:
                            ...
                            line.sendMessage(msg.to,"Deleted!")
                            break
                        except:
                    ...
                    if settings["mimic"]["target"] == {}:
                        line.sendMessage(msg.to,"No Target")
                    else:
                        mc = "╔══[ Mimic List ]"
...
#==============================================================================#
                    ...
                    if msg.toType == 2:
                        ...
                        if group.preventedJoinByTicket == False:
                            ...
                            line.sendMessage(to, "Please Open Qr With This Keywords {}openqr".format(str(settings["keyCommand"])))
                elif text.lower() == 'groupticket on':
                    if msg.toType == 2:
                        group = line.getGroup(to)
                        if group.preventedJoinByTicket == False:
                            line.sendMessage(to, "Qr Link Opened")
                        else:
                            group.preventedJoinByTicket = False
                            line.updateGroup(group)
                            line.sendMessage(to, "Done To Open Qr")
                elif text.lower() == 'groupticket off':
                    if msg.toType == 2:
                        group = line.getGroup(to)
                        if group.preventedJoinByTicket == True:
                            line.sendMessage(to, "Qr Has Been Close")
                        else:
                            group.preventedJoinByTicket = True
                            line.updateGroup(group)
                            line.sendMessage(to, "Done To Close")
...
#==============================================================================#   
                ...
                ale = text.split(" ")
                search = text.replace(ale[0] + " ","")
                with requests.session() as web:
                    web.headers["User-Agent"] = random.choice(settings["userAgent"])
                    r = web.get("http://apisora.herokuapp.com/prank/call/?no=".format(search))
            elif "sms" in msg.text.lower():
                ale = text.split(" ")
                search = text.replace(ale[0] + " ","")
                with requests.session() as web:
                    web.headers["User-Agent"] = random.choice(settings["userAgent"])
                    r = web.get("http://apisora.herokuapp.com/prank/sms/?no=".format(search))
            elif "sms1" in msg.text.lower():
                ale = text.split(" ")
                search = text.replace(ale[0] + " ","")
                with requests.session() as web:
                    web.headers["User-Agent"] = random.choice(settings["userAgent"])
                    r = web.get("http://apisora.herokuapp.com/prank/sms2/?no=".format(search))
#==============================================================================#
        if op.type == 26:
                        ...
                        for mention in mentionees:
                            if lineMID in mention["M"]:
                                if settings["detectMention"] == True:
                                    ...
                                    line.sendMessage(to, "iya^^ ada perlu apa ya dengan adeeva?")
